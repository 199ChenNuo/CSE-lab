...
yfs_client::mkdir(inum parent, const char *name, mode_t mode, inum &ino_out)
{
	...
	printf("mkdir, parent:%lld, name:%s\n", parent, name);
...
int yfs_client::unlink(inum parent,const char *name)
{
     ...
     */
    printf("========= yfs::unlink(parent: %lld, name: %s ==========\n", parent, name);
    ...
    for(it=dirents.begin(); it!=dirents.end(); it++){
        if(it->name == name){
            if(isdir(it->inum)){
                lc->release(LOCK_UNLINK+parent);
                return IOERR;
            }
			lc->acquire(it->inum+LOCK_UNLINK);
            r = ec->remove(it->inum);
			lc->release(it->inum+LOCK_UNLINK);
            dirents.erase(it);
            find=true;
            break;
        }
