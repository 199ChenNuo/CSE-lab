...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
	...
	char padc;
	// positive flag, for number like +(minus) should not display '+'
	int posflag;
	...
	reswitch:
		...
		case 'd':
			...
			if ((long long) num < 0) {
				putch('-', putdat);
				// !!!!!! set positive flag = 0 and ignore the '+' 
				// that intented to be printed in front of a negtive value
				posflag = 0;
				num = -(long long) num;
			} 
			base = 10;
		...
		case 'n': {
			...
			char* cp = va_arg(ap, char *);
    			...
    			*cp = *(int *)putdat;
    			// 127(0x7f) is the biggest value that 1 signed byte can present
    			if ((*(int *) putdat) > 127)
