...
boot_alloc(uint32_t n)
{
	...
	// PADDR(): translate kernal-VA to kernal-PA 
	if(PADDR(nextfree) > npages*PGSIZE){
		panic("boot_alloc: out of memory\n");
	}
	else if((unsigned long)nextfree < KERNBASE){
		panic("boot_alloc: out of memory\n");
	}
	return result;
}
...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	// KERNBASE: 0xf0000000, so should turn it to positive
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// if page table not exist
	else {
		// 	if create is flase, return NULL
		if (!create) return NULL;
		// 	else create page table
		// 		return the new page table page
		pageInfo = page_alloc(ALLOC_ZERO);
		if (!pageInfo) return NULL;
		pageInfo->pp_ref++;
		// set permission
		pte_addr = page2pa(pageInfo);
		pgdir[PDX(va)] = pte_addr | PTE_U | PTE_W | PTE_P;
		pgtbl = KADDR(pte_addr);
	}
	return &pgtbl[PTX(va)];
}
...
{
	// cprintf("boot_map_region_large\n");
	// Fill this function in
	if (!pgdir) panic("boot_map_region_large: null pointer 'pgdir'\n");

	size = ROUNDUP(va + size, PTSIZE) - ROUNDDOWN(va, PTSIZE);
	va = ROUNDDOWN(va, PTSIZE);
	pa = ROUNDDOWN(pa, PTSIZE);
	// size is multiple of (PGSIZE * NPTENTRIES)
	size_t pos;
	for(pos=0; pos<size; pos+=PTSIZE){
		// pte_t *pte = pgdir_walk(pgdir, (void *)(va+pos), true);
		// if(!pte) panic("boot_map_region_large: pgdir_walk return NULL\n");
		// (*pte) = (pa+pos) | perm | PTE_P | PTE_PS;
		pgdir[PDX(va+pos)] = (pa+pos) | perm | PTE_P | PTE_PS;
	}
}
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	// struct PageInfo *pageInfo = (struct PageInfo*)boot_alloc(1);
	va = ROUNDDOWN(va, PGSIZE);
