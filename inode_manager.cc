...
block_manager::alloc_block()
{
  ...
  for(bitmapno=2; bitmapno<BBLOCK(BLOCK_NUM); bitmapno++){
    ...
    // printf("====line43 bitmapno: %d ====\n", bitmapno);
    for(char_idx=0; char_idx<BLOCK_SIZE; char_idx++){
      bit = bitmap[char_idx];
      // printf("====line46 char_idx: %d, bit: %x ====\n", char_idx, bit);
      if(bit != 0xf){
        for(bit_idx=0; bit_idx<BYTE_SIZE; bit_idx++){
          if(! (bit & (mask << bit_idx))){
            // printf("==== bit_idx: %d, mask: 0x%x ====\n", bit_idx, mask<<bit_idx);
            // printf("bm: alloc_block, id: %d\n\n", bitmapno*BPB + char_idx*BYTE_SIZE + bit_idx);
...
inode_manager::alloc_inode(uint32_t type)
{
  ...
  for(inum = 1; inum < bm->sb.ninodes; inum++){
    ...
    if(inode->type == 0){
      ...
      // bzero(inode->blocks, NDIRECT+1);

      put_inode(inum, inode);

      printf("\tim: alloc_inode, inum: %d\n", inum);
      printf("end of im::alloc_inode()\n\n");
      return inum;
    }
  }
...
inode_manager::read_file(uint32_t inum, char **buf_out, int *size)
{
  ...
  *buf_out = (char *)malloc(ino->size);
  if(bsize > NDIRECT){
    ...
    if(has_tail){
      bm->read_block(ndir_buf[bsize], buf);
...
inode_manager::write_file(uint32_t inum, const char *buf, int size)
{
  ...
  // printf("\tsize:%d, old_size:%d, old_bsize:%d, total new_bsize:%d\n", size, ino->size, old_bsize, new_bsize+has_tail);
  ...
  if(new_bsize+has_tail > NDIRECT){
    // printf("\t line369 total new_bsize > ndriect\n");
    for(i=0; i<NDIRECT; i++){
    ...
    // printf("\t line377 total new_bsize < ndriect\n");
    for(i=0; i<new_bsize; i++){
      bm->write_block(ino->blocks[i], buf+i*BLOCK_SIZE);
    }
    if(has_tail){
      // printf("\t write tail data\n");
      memcpy(tail_buf, buf + new_bsize * BLOCK_SIZE, size%BLOCK_SIZE);
      // printf("\t after memcpy\n");
  ...
  ino->size = size;
  ino->atime = ino->ctime = (unsigned int)time(NULL);
  put_inode(inum, ino);
